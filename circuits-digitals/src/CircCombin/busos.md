...
On $A_3$ és el bit de més pes (*Most Significant Bit o **MSB***) i $A_0$ és el bit de menys pes (*Less Significant Bit o **LSB***).
Aquesta notació permet referir-nos tant al conjunt complet ($A$) com a cadascun dels seus bits ($A_i$).
...
A continuació es mostra la taula de veritat d’un circuit desplaçador a l’esquerra. L’entrada és $A[3:0] = [A_3 A_2 A_1 A_0]$ i la sortida és $B[3:0] = [B_3 B_2 B_1 B_0]$.
...
* Usar registres de desplaçament, que són [circuits seqüencials](../CircSeq/intro.md) i adequats per a desplaçaments successius sincronitzats amb rellotge.

En aquesta secció utilitzarem multiplexors. El circuit és el següent:

<img src="./bus_exemple.png" alt="Circuit desplaçador" style="display:block; height:400px; margin:0 auto; border-radius:8px; background-color:white; padding:4px;"/>
...
Per a cada bit de sortida $B_i$, utilitzarem un multiplexor 2-a-1:
